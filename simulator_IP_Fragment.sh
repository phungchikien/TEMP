...
# =================================================================
#             IP FRAGMENT FLOOD FOR EDUCATIONAL PURPOSE
...
LOG_FILE="hping3_ip_fragment_flood_$(date +%Y%m%d_%H%M%S).log"
VERBOSE=true
...
generate_compressed_pattern_python() {
    ...
    log "=== IP FRAGMENT FLOOD SIMULATOR ==="
...
generate_yoyo_pattern_python() {
    ...
    log "=== IP FRAGMENT FLOOD YO-YO PATTERN ==="
...
main() {
    ...
    log "=== IP FRAGMENT FLOOD SIMULATOR WITH SAFE TC DETECTION ==="
...
if [[ "$1" == "-h" || "$1" == "--help" ]]; then
    echo "IP Fragment Flood Simulator with Safe TC Detection"
